...
class MultimodalRetriever:
    ...
    def _get_image_description(self, file_path: str, filename: str) -> str:
        """
        Generate a text description for an image.
        Uses a cached description file if available, otherwise falls back to filename-based description.
        A richer description can be generated by an LLM vision call from the workflow layer.
        """
        # Check for a sidecar description file  e.g. image.jpg -> image.jpg.desc
        desc_file = file_path + ".desc"
        if os.path.exists(desc_file):
            try:
                ...
                pass
        # Fallback: use file stem as description
        stem = Path(filename).stem
        # Replace underscores/hyphens with spaces for readability
        return stem.replace("_", " ").replace("-", " ")

    def _load_images(self, folder_path: str) -> List[Document]:
        """Load images from folder and index them with text descriptions for semantic retrieval."""
        image_docs = []
        ...
        for file in sorted(os.listdir(folder_path)):
            if Path(file).suffix.lower() in image_extensions:
                file_path = os.path.join(folder_path, file)
                description = self._get_image_description(file_path, file)
                image_doc = Document(
